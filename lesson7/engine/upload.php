...
function upload()
{
    ...
    if (!empty($_FILES)) {
        ...
        $pathToFile = ROOT . "/images/gallery/big/" . $pictureName;
        ...
        if (move_uploaded_file($_FILES['picture']['tmp_name'], $pathToFile)) {
            ...
            $image->save(ROOT . "/images/gallery/small/" . $pictureName);
            // Удалить исходную картинку, если не удалось сохранить миниатюру
            if (empty(glob(ROOT . "/images/gallery/small/" . $pictureName))) {
                unlink($pathToFile);                                    
            ...
            if (!mysqli_query(getDb(), $sql)) {
                unlink($pathToFile);                                    
                unlink(ROOT . "/images/gallery/small/" . $pictureName);
